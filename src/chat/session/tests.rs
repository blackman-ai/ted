...
#[test]
fn test_record_message_and_persist_sets_first_summary() {
    let mut history_store = HistoryStore::open().unwrap();
    let session_id = SessionId::new();
    let mut session_info = SessionInfo::new(session_id.0, std::env::current_dir().unwrap());
    ...
    assert_eq!(session_info.summary.as_deref(), Some("hello world"));

    let persisted = history_store.get(session_id.0).unwrap();
    assert_eq!(persisted.message_count, 1);
}

#[test]
fn test_record_message_and_persist_without_summary() {
    let mut history_store = HistoryStore::open().unwrap();
    let session_id = SessionId::new();
    let mut session_info = SessionInfo::new(session_id.0, std::env::current_dir().unwrap());
    let mut message_count = 0;
    ...
    assert_eq!(message_count, 1);
    assert_eq!(session_info.message_count, 1);
    assert!(session_info.summary.is_none());
}

#[test]
fn test_trim_conversation_if_needed_empty() {
    let provider = MockProvider::new();
    let mut conversation = Conversation::new();

    let removed = trim_conversation_if_needed(&provider, "mock-model", &mut conversation);
...
fn test_chat_session_builder_default_values() {
    ...
    assert!(builder.session_state.is_none());
    assert!(builder.files_in_context.is_empty());
...
fn test_builder_with_caps_single() {
    let settings = Settings::default();
    let builder = ChatSessionBuilder::new(settings).with_caps(vec!["code-assistant".to_string()]);
...
#[test]
fn test_builder_with_session_state() {
    let settings = Settings::default();
    let session_id = SessionId::new();
    let session_info = SessionInfo::new(session_id.0, PathBuf::from("/tmp"));
    ...
    let builder = ChatSessionBuilder::new(settings).with_session_state(session_state);
    let state = builder
        .session_state
        .as_ref()
        .expect("session state should be set");
    assert_eq!(state.session_id.0, session_id.0);
    assert_eq!(state.message_count, 7);
    assert!(state.is_resumed);
}

#[test]
fn test_builder_with_files_in_context() {
    let settings = Settings::default();
    let files = vec!["src/main.rs".to_string(), "Cargo.toml".to_string()];
    let builder = ChatSessionBuilder::new(settings).with_files_in_context(files.clone());

    assert_eq!(builder.files_in_context, files);
}
...
fn test_builder_with_provider_sets_name() {
    ...
    impl crate::llm::provider::LlmProvider for MockProvider {
        ...
        async fn complete(
            ...
            Err(crate::error::TedError::Internal(
                "MockProvider::complete is not used in this test".to_string(),
            ))
        }
        ...
        > {
            Err(crate::error::TedError::Internal(
                "MockProvider::complete_stream is not used in this test".to_string(),
            ))
        }
...
fn test_builder_working_directory_absolute() {
    let settings = Settings::default();
    let builder =
        ChatSessionBuilder::new(settings).with_working_directory(PathBuf::from("/absolute/path"));
...
fn test_builder_working_directory_relative() {
    let settings = Settings::default();
    let builder =
        ChatSessionBuilder::new(settings).with_working_directory(PathBuf::from("relative/path"));
...
fn test_builder_working_directory_with_dots() {
    let settings = Settings::default();
    let builder =
        ChatSessionBuilder::new(settings).with_working_directory(PathBuf::from("../parent/path"));
...
fn test_builder_with_model_special_chars() {
    let settings = Settings::default();
    let builder =
        ChatSessionBuilder::new(settings).with_model("model-with-dashes_and_underscores:version");
...
impl crate::llm::provider::LlmProvider for TestMockProvider {
    ...
    async fn complete_stream(
        ...
        std::pin::Pin<
            Box<
                dyn futures::Stream<Item = crate::error::Result<crate::llm::provider::StreamEvent>>
                    + Send,
            >,
        >,
    > {
...
#[tokio::test]
async fn test_build_with_explicit_session_state() {
    let settings = Settings::default();
    let provider = Arc::new(TestMockProvider::new());
    let session_id = SessionId::new();
    let session_info = SessionInfo::new(session_id.0, std::env::temp_dir());
...
fn test_resume_session_from_history_empty_string() {
    ...
    let result = ChatSessionBuilder::resume_session_from_history(&history_store, "", &working_dir);
