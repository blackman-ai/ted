...
/// Increment message count, optionally set initial summary, and persist session metadata.
pub fn record_message_and_persist(
    history_store: &mut HistoryStore,
    session_info: &mut SessionInfo,
    message_count: &mut usize,
    summary_candidate: Option<&str>,
) -> Result<()> {
    *message_count += 1;

    if *message_count == 1 {
        if let Some(summary) = summary_candidate {
            session_info.set_summary(summary);
        }
    }
...
/// Proactively trim conversation near model context limit to reduce future latency.
pub fn trim_conversation_if_needed(
...
/// Explicit session state for session initialization.
#[derive(Debug, Clone)]
...
impl ChatSessionBuilder {
    ...
    /// Provide explicit session state instead of creating/resuming internally.
    pub fn with_session_state(mut self, session_state: SessionState) -> Self {
        self.session_state = Some(session_state);
        self
    }

    /// Include files in the tool context for this session.
    ...
    pub async fn build(self) -> Result<ChatSession> {
        ...
        let model = self.model.unwrap_or_else(|| {
            merged_cap
                ...
                .map(|s| s.to_string())
                .unwrap_or_else(|| {
                    ProviderFactory::default_model(&self.provider_name, &self.settings)
                })
        });
        ...
        let (session_id, mut session_info, message_count, is_resumed) = if let Some(session_state) =
            self.session_state
        {
            (
                ...
                session_state.is_resumed,
            )
        } else if let Some(ref resume_id) = self.resume_id {
        ...
        let context_storage_path = self.settings.context.storage_path.clone();
        let mut context_manager =
            match ContextManager::new(context_storage_path, session_id.clone()).await {
                Ok(manager) => manager,
                #[cfg(test)]
                Err(crate::error::TedError::Io(err))
                    if err.kind() == std::io::ErrorKind::PermissionDenied =>
                {
                    let fallback_path = std::env::temp_dir().join("ted-test-context");
                    ContextManager::new(fallback_path, session_id.clone()).await?
                }
                Err(e) => return Err(e),
            };
        ...
        // Persist project root and caps metadata on the session.
        session_info.project_root = project_root.clone();
        if !is_resumed || session_info.caps.is_empty() {
            session_info.caps = self.cap_names.clone();
        }
        ...
        .with_files_in_context(self.files_in_context.clone());
