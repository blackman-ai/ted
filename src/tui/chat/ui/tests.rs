...
async fn test_render_confirm_dialog_long_message() {
    let mut app = create_test_chat_app().await;
    app.confirm_message =
        Some("Are you absolutely sure you want to delete this very important file?".to_string());
...
async fn test_draw_with_tool_calls() {
    ...
    let mut msg =
        crate::tui::chat::state::DisplayMessage::assistant("Reading file...".to_string(), vec![]);
